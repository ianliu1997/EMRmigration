...
namespace PalashDynamics.Web
{
    ...
    public class DataTemplateHttpsService
    {
        [OperationContract]
        public void DoWork()
        {
        ...
        public void UploadReportFileForRadiology(string fileName, byte[] data)
        {
            ...
            using (FileStream fs = new FileStream(file, FileMode.Create))  // Changed from  CreateNew => Create on 05122017     // using (FileStream fs = new FileStream(file, FileMode.CreateNew))
            {
                fs.Write(data, 0, (int)data.Length);
            }
        }
