namespace PalashDynamics.DataAccessLayer.BaseDataAccessLayer.Administration
{
    ...
    {
        private static clsBaseServiceMasterDAL _instance;

        protected clsBaseServiceMasterDAL()
        {
        }
        ...
        public static clsBaseServiceMasterDAL GetInstance()
        {
            try
            {
                if (_instance == null)
                {
                    string str = "clsServiceMasterDAL";
                    _instance = (clsBaseServiceMasterDAL) Activator.CreateInstance(Type.GetType(HMSConfigurationManager.GetDataAccesslayerNameSpace() + "." + str), true);
                }
            }
            catch (Exception)
            {
