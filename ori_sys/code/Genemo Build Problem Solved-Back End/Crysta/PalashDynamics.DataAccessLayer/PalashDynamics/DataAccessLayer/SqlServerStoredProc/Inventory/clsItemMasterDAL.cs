namespace PalashDynamics.DataAccessLayer.SqlServerStoredProc.Inventory
{
    ...
    public class clsItemMasterDAL : clsBaseItemMasterDAL
    {
                ...
                if (reader.HasRows)
                {
                    ...
                    while (reader.Read())
                    {
                        ...
                        if (!nvo.IsFromOpeningBalance && (nvo.StoreID != 0L))
                        {
                            ...
                            if (nvo.PackageID < 0L)
                            {
                                long packageID = nvo.PackageID;
                            }
                            if (nvo.PackageID > 0L)
                            {
                                item.PckgItmQuantity = Convert.ToInt32(DALHelper.HandleDBNull(reader["Quantity"]));
                            }
                        }
        ...
        public override IValueObject GetPackageItemQuantityList(IValueObject valueObject, clsUserVO uservo)
        {
            clsGetItemListForSearchBizActionVO nvo = valueObject as clsGetItemListForSearchBizActionVO;
            try
            {
                DbCommand storedProcCommand = this.dbServer.GetStoredProcCommand("GetPackageItemUsedQuantity");
                ...
                if (reader.HasRows)
                {
                    ...
                    {
                        clsItemMasterVO item = new clsItemMasterVO {
                            ID = Convert.ToInt64(DALHelper.HandleDBNull(reader["ItemID"])),
                            UsedQuantity = Convert.ToInt64(DALHelper.HandleDBNull(reader["UsedQty"])),
                            BatchId = new long?(Convert.ToInt64(DALHelper.HandleDBNull(reader["BatchId"])))
                        };
                        nvo.ItemList.Add(item);
                    }
